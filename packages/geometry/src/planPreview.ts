// Pure Model helper for the plan preview + downloads. Takes a models map
// ({ <id>: { name, widthCm, depthCm, svg } }) and a lead's placed `items`
// ([{ modelId, x, y, rot, ... }]) and produces:
...
import type { DxfPlacement } from './planToDxf.ts';

/** A catalogue model as the preview needs it. */
...
/** A placed item: (x,y) = footprint-AABB top-left in plan cm, rot in degrees. */
...
export type PlanPreviewModels = Record<string, PlanPreviewModel>;

export interface PlacedPieceGeom {
  W: number;
  H: number;
  vbW: number;
  vbH: number;
...
export interface PlanPreviewOptions {
  /** Blank cm around the layout's bounding box; default 24. */
  margin?: number;
}
...
export function viewBoxOf(svg: unknown): { vbW: number; vbH: number } {
...
function innerSvg(svg: unknown): string {
 ...
 */
export function placedPieceGeom(
  model: PlanPreviewModel | null | undefined,
  item: PlanPreviewItem | null | undefined,
): PlacedPieceGeom {
 ...
 * payload. A missing model still contributes its position (planToDxf falls back to
 * the footprint rectangle from widthCm/depthCm). Returns [] when there is nothing
 * to export.
 */
export function planPlacements(
  models: PlanPreviewModels | null | undefined,
  items: Array<PlanPreviewItem> | null | undefined,
): DxfPlacement[] {
  return (items || [])
    .filter((it) => it && it.modelId != null)
    .map((it) => {
      const m = (models || {})[String(it.modelId)] || {};
      return {
        pieceId: it.modelId as string | number,
        x: num(it.x),
        y: num(it.y),
        rot: num(it.rot),
...
/**
 * Compose the monochrome top-down plan SVG for a layout. Returns
 ...
 */
export function composePlanPreview(
  models: PlanPreviewModels | null | undefined,
  items: Array<PlanPreviewItem> | null | undefined,
  opts: PlanPreviewOptions = {},
): PlanPreview | null {
  const margin = num(opts.margin, 24);
  const byId = models || {};
  const placed = (items || [])
    .filter((it) => it && byId[String(it.modelId)])
    .map((it) => {
      const model = byId[String(it.modelId)];
      return { it, model, geom: placedPieceGeom(model, it) };
    })
