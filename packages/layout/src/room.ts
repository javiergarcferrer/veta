// The room / space boundary the client works within — pure geometry (no React,
// no three, no DOM). A room is a CLOSED POLYGON in plan centimetres, the SAME
// frame every placed piece lives in: plan-x → world-X, plan-y → world-Z,
// 1 cm = 1 world unit. A rectangle is just the 4-corner special case
// (`shape: 'rect'`); a traced outline is `shape: 'poly'`.
//
// VISUAL REFERENCE ONLY: nothing here constrains placement. The room is drawn on
// the floor so the customer can eyeball the fit and the dealer reads the client's
// real space; `roomFit` reports overflow but never blocks a drag.
//
// Pure data-in/data-out so it's unit-testable and shared by the persistence
// (buildShare), the 3D render (the room floor group) and the fit readout.

export type RoomShape = 'rect' | 'poly';

export interface RoomCorner { x: number; y: number }
...
export interface RoomBounds { minX: number; minY: number; maxX: number; maxY: number }
...
export function clampRoomDim(n: unknown): number {
  const v = Number(n);
 ...
 */
export function makeRectRoom(
  widthCm: unknown,
  depthCm: unknown,
  { cx = 0, cy = 0 }: { cx?: number; cy?: number } = {},
): Room {
...
export function normalizeRoom(room: Partial<Room> | null | undefined): Room | null {
...
export function roomBounds(room: Partial<Room> | null | undefined): RoomBounds | null {
  const cs = room?.corners;
...
export function roomSize(room: Partial<Room> | null | undefined): { widthCm: number; depthCm: number } | null {
...
export function roomCenter(room: Partial<Room> | null | undefined): RoomCorner | null {
  const b = roomBounds(room);
  return b ? { x: r2((b.minX + b.maxX) / 2), y: r2((b.minY + b.maxY) / 2) } : null;
}

/** Even-odd ray cast: is plan point {x,y} inside the room polygon? */
export function pointInRoom(room: Partial<Room> | null | undefined, pt: RoomCorner | null | undefined): boolean {
  const cs = room?.corners;
 ...
 * {minX,minY,maxX,maxY}. Returns { fits, overflowCm } where overflowCm is the
 * largest amount any side of the design pokes past the room's bounding box
 * (0 = fully inside). Null when there is no room or nothing placed. A
 * bounding-box test (not per-edge): a forgiving, legible "does it roughly fit"
 * that matches how a person eyeballs a plan.
 */
export function roomFit(
  room: Partial<Room> | null | undefined,
  designBounds: RoomBounds | null | undefined,
): { fits: boolean; overflowCm: number } | null {
