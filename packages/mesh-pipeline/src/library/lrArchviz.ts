/**
 * LIBRARY LAYOUT #1 — Ligne Roset's ARCHVIZ tree.
 *
 * The dealer drags LR's whole ARCHVIZ library onto the studio (thousands of
 * files, one drop) and every one of them has to land under three levels without
 * anybody typing a word:
 *
 *   GROUP       LR's top division — Upholstery, Dining, Bedroom…
 *   CATEGORY    what the piece IS  — Sofa, Beds, Coffee Tables…
 *   COLLECTION  the model family   — Togo, Prado, Hypna…
 *
 ...
 *
 * This replaced a vocabulary-driven parser (an `LR_CATEGORIES` list + a
 * CATEGORY→GROUP table) that read every folder name against enumerated words.
 ...
 *     against a list; whatever sits at its index IS the category.
 *   • A GROUP LEVEL CAN BE ABSENT PER BRANCH. LR's own tree hangs `Cabinet/`
 *     beside `Upholstery/` — the owner's rule verbatim: «cabinets should all be
 *     categories. no parent group.» A segment at the group index that carries no
 *     group token is therefore the CATEGORY of a branch that has no group, and
 *     the answer is `group: null` — a blank the dealer fills in one click, never
 *     a guessed division he has to notice first.
 ...
 *     category, by being the last thing standing — but they are REPORTED as
 *     `variant`, so nothing about the path is silently thrown away.
 *
 * Every taxonomy field is `string | null` — null means "the path doesn't say",
 * never a guess, because a guessed GROUP is a piece the dealer can never find
 * again. Pure and throw-free: the drop handler, the importer and the taxonomy UI
 * all read the SAME answer, and the answer is a pure function of (string,
 * shape), so re-dropping the same folder can never file the same file twice.
 */
import type { LibraryLayout, LibraryPathFields } from './types.ts';
import { noLibraryFields } from './types.ts';

/**
 * LR's top division. Exported because a UI offers these as the GROUP picker's
 ...
 * (this is LR's own vocabulary, not app copy); localized labels belong to the
...
const keyOf = (s: unknown): string =>
  String(s == null ? '' : s).replace(/[-_\s]+/g, ' ').trim().toUpperCase();
...
const tokensOf = (s: unknown): string[] =>
  String(s == null ? '' : s).split(/[-_\s]+/).filter(Boolean);
...
const isVariantFolder = (key: string): boolean =>
  VARIANT_KEYS.has(key) || VARIANT_SHAPES.some((re) => re.test(key));
...
const opensWith = (key: string, prefix: string): boolean =>
  !!prefix && !!key && (key === prefix || key.startsWith(`${prefix} `));
...
const groupSplit = (segment: string): { group: string; rest: string } | null => {
...
const splitPath = (relPath: unknown): string[] => (typeof relPath !== 'string' ? [] : relPath
  .replace(/\\/g, '/').split('/')
  .map((s) => s.trim())
  .filter((s) => s && s !== '.'));

// The FOLDER segments, roots and drive stubs dropped wherever they appear. A
// variant folder is still IN here — it is dropped from the level count below,
// but it is also the answer to `variant`, and nothing about the path is thrown
// away silently.
const folderSegments = (segments: string[]): string[] =>
  segments.slice(0, -1).filter((s) => !ROOT_KEYS.has(keyOf(s)) && !DRIVE.test(s));

// The folders a level can be counted at: trailing VARIANT folders dropped (a
// rendering is never a level — and it always trails, since it sits under the
// model folder). Shape resolution and the per-path read share this one cleaner,
// so the indices the batch counts are exactly the ones a path reads.
const cleanFolders = (segments: string[]): string[] => {
  const folders = folderSegments(segments);
 ...
 */
export function prettifyLibraryName(raw: unknown): string {
...
/**
 * The plan `parseLibraryPath` reads paths by. OPAQUE — treat it as a token to
 * hand back, not as fields to read.
 *
 ...
 *   categoryDepth  where the CATEGORY sits: groupDepth + 1 normally, groupDepth
...
export interface LibraryShape {
  readonly groupDepth: number | null;
  readonly categoryDepth: number;
  readonly compound: boolean;
}

/**
 * Infer ONE batch's SHAPE.
 *
 ...
 */
export function resolveLibraryShape(relPaths: string[]): LibraryShape {
  const votes = new Map<number, { plain: number; compound: number }>();
  for (const relPath of Array.isArray(relPaths) ? relPaths : []) {
    const folders = cleanFolders(splitPath(relPath));
    let at = -1;
    let split: { group: string; rest: string } | null = null;
    for (let i = 0; i < folders.length && at < 0; i++) {
      split = groupSplit(folders[i]);
      if (split) at = i;
    }
    if (at < 0 || !split) continue;   // this path has no group level — it votes for nothing
...
const usableShape = (shape: unknown): LibraryShape | null => {
  const s = shape as LibraryShape | null;
  return (s && typeof s === 'object'
    && (s.groupDepth === null || (Number.isInteger(s.groupDepth) && (s.groupDepth as number) >= 0)))
    ? s
    : null;
};

/**
 * File a library path under { group, category, collection } — plus the packaging
 * it read them past — reading it BY INDEX against the batch's shape.
 *
 ...
 *      one segment, the remainder is the category), or carries none at all —
 *      and then this branch has no group, so that segment IS the category
 ...
 */
export function parseLibraryPath(relPath: string, shape?: LibraryShape): LibraryPathFields {
  if (typeof relPath !== 'string') return noLibraryFields();

  const segments = splitPath(relPath);
  if (!segments.length) return noLibraryFields();
  ...
  // ── 2. The folders a level can be counted at, and the packaging read past.
  const folders = cleanFolders(segments);
  const variant = folderSegments(segments).find((f) => isVariantFolder(keyOf(f))) || '';
  ...
  if (folders.length) {
    if (groupDepth == null) {
      ...
      if (!split) {
        ...
        // the taxonomy starts here, and the group stays blank rather than
  ...
  return {
    // The group is emitted VERBATIM from LR_GROUPS (never re-cased through the
    ...
    modelFolder: orNull(prettifyLibraryName(modelFolder)),
    // Packaging, verbatim — it is reported so the path is fully accounted for,
    // never presented as a name.
    variant: orNull(variant),
    fileName: orNull(file),
  };
}

/** Ligne Roset ARCHVIZ, as a `LibraryLayout` adapter. */
export const lrArchvizLayout: LibraryLayout<LibraryShape> = {
  id: 'lr-archviz',
  label: 'Ligne Roset ARCHVIZ',
  resolveShape: resolveLibraryShape,
  parsePath: parseLibraryPath,
};
