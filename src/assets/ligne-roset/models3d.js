 ...
 * them. So the 3D view ships with procedural geometry (lib/configurator/model.js)
...
export function glbForPiece(piece) {
  ...
  return CONFIGURATOR_GLB[fp] || (piece.kind && CONFIGURATOR_GLB[piece.kind]) || null;
}
...
export function glbFor(kind) {
  return (kind && CONFIGURATOR_GLB[kind]) || null;
}

/** Whether ANY real Togo model is wired (so the View can flag "real models on"). */
export function hasConfiguratorGlb() {
  return Object.keys(CONFIGURATOR_GLB).length > 0;
}
