/**
 * Carl Hansen & Søn — which VARIANT a configuration is.
 *
 * Ported from the dealer back-office, minus its row builder: a configurator
 * does not mint catalog rows, it answers "which EAN am I looking at, what does
 * it cost, and when does it arrive". Those three answers are here.
 *
 * A variant is one EAN, one configuration,
...
const LABEL_ALIASES: ReadonlyArray<readonly [readonly string[], string]> = [
  ...
  // THE CERTIFICATION GOT RE-WORDED ON ONE SIDE ONLY. The blob master still
  // says "FSC™-certified Oak"; the product page now prints "FSC™ Mix-certified
  // oak". Same wood, same EAN — and with the two sides disagreeing, EVERY CH24
  // configuration matched zero variants, so the configurator showed no
  // reference and no lead time at all.
  //
  ...
  // and deleting "FSC…certified" would fold them into one. Collapsing the one
  // word that moved keeps them apart.
  [['fsc', 'mix'], 'fsc'],
];
...
/**
 * Lead times for exactly the variants `variantMatchesSelection` accepts for
