  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-brand-200 bg-surface dark:border-brand-900/50">
      ...
      <div className="flex shrink-0 flex-wrap items-center gap-x-2 gap-y-1 border-b border-ink-200 px-3 py-2">
        ...
        <button type="button" onClick={onClose} disabled={binding} className="btn-ghost ml-auto text-micro disabled:opacity-50" title="Cerrar la revisión sin vincular nada">
          <X size={12} /> Cerrar
        </button>
      </div>
      ...
      {stage === 'done' && (
        <>
          ...
          {(degraded || duplicates.length > 0 || dropped.length > 0 || log.length > 1) && (
            <div className="shrink-0 space-y-1 border-b border-ink-100 bg-ink-50/60 px-3 py-2 text-micro leading-tight text-ink-500 dark:bg-ink-100/5">
              {degraded && (
            ...
            {rows.map((r) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <div className="grid h-9 w-12 shrink-0 place-items-center overflow-hidden rounded bg-ink-50 text-ink-500">
                    {thumbs[r.modelId]
                      ? <img src={thumbs[r.modelId]} alt="" draggable={false} className="h-full w-full select-none object-contain" />
                      : <span className="text-micro">3D</span>}
                  </div>
                  ...
                  <div className="min-w-0 flex-1 space-y-1">
                    <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                      ...
                      {conf && <span className={`rounded-full px-1.5 py-0.5 text-micro font-medium ${conf.chip}`}>{conf.label}</span>}
                      {r.source === 'fallback' && (
                        <span className="rounded-full bg-ink-200 px-1.5 py-0.5 text-micro text-ink-600">sin Claude</span>
                      )}
                      {r.duplicate && (
                        <span className="rounded-full bg-amber-500/15 px-1.5 py-0.5 text-micro text-amber-700 dark:text-amber-300" title="Esta referencia también se propuso para otra pieza">
                          repetida
                        </span>
                      )}
                    </div>
                    ...
                    {r.options.length > 0 ? (
                      <div className="flex flex-wrap items-center gap-x-2 gap-y-1" onClick={(e) => e.stopPropagation()}>
                        ...
                        <Select
                          variant="chip"
                          className="max-w-full !min-h-7 !py-0.5 !text-micro"
                          value={root}
                        ...
                        <span className="shrink-0 text-micro font-medium tabular-nums text-ink-700">{usdFrom(picked?.fromUsd ?? null)}</span>
                      </div>
                    ) : (
                      <p className="text-micro text-ink-500">Sin candidatos admisibles en el catálogo.</p>
                    )}

                    {r.why && <p className="text-micro leading-tight text-ink-500">{r.why}</p>}
                  </div>
                </div>
              );
            })}
