...
import { buildArGroup, exportGlbBlob, loadSceneFabrics } from './glbExport.js';
import { loadConfiguratorModels } from './modelLoader.js';
import { disposeGroup as disposeModel } from './sceneBuilder.js';
 ...
 */
export default function ConfiguratorArViewer({ open, onClose, scene3d, material, storeName, phoneUrl, fabricByCode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // model falls back to procedural geometry inside buildConfiguratorGroup.
        //
        ...
        const [{ fabricTextures, colors, pbr, normals, extras, dispose: disposeFabrics }, { cache: modelCache, modelFor }] = await Promise.all([
          ...
          loadConfiguratorModels(sd, new Map()),
        ]);
