...
import { inferConfiguratorForm, CONFIGURATOR_HEIGHT_CM } from '../../lib/configurator/model.js';
...
export default function ConfiguratorStage({
  ...
  const buildScene = useCallback(() => {
    ...
    const pieces = (pl || []).map((p) => {
      ...
      return {
        uid: p.uid, pieceId: p.pieceId, widthCm: w, depthCm: d, form: inferConfiguratorForm(r.label || r.name, w, d),
  ...
  const poseFor = useCallback((m, center, radius, aspect, netInset = 0, margin3d = MARGIN_3D_LAYOUT, netInsetV = 0) => {
    ...
    return m === '2d'
      ...
      ? (() => {
        const camY = fitDist + CONFIGURATOR_HEIGHT_CM;               // the height it really flies at
      ...
      : (() => {
        const camY = fitDist3d * 0.46 + CONFIGURATOR_HEIGHT_CM * 0.5;
  ...
  const placeCamera = useCallback((l, pose, m) => {
    ...
    if (m === '2d') {
      ...
      controls.maxDistance = fitDist * 2.6 + CONFIGURATOR_HEIGHT_CM;
  ...
  const rebuild = useCallback(async () => {
    ...
    const [, loaded] = await Promise.all([
      ...
      loadConfiguratorModels({ pieces: scene.pieces }),
    ]);
    ...
    const group = buildConfiguratorGroup(l.deps, scene, {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const renderNow = () => {
        ...
        // The ground shadow is a BAKED buffer now (setupConfiguratorStage → contact
      ...
      const { dispose: disposeStage, retarget: retargetStage, setShadow: setShadowMode, updateShadow } = setupConfiguratorStage(deps, renderer, scene, 320, { grid: true, ...(stateRef.current.ground ? { ground: stateRef.current.ground } : {}) });
      // The plan grounds the piece with a contact shadow from overhead; the
      // raking key's long shadow is a 3D-vista read (see setupConfiguratorStage.setShadow).
    ...
    return () => {
      ...
      if (l) {
        ...
        // modelLoader's sharedModelCache), so this teardown must not free
  ...
  return (
    ...
    <div style={{ position: 'absolute', inset: 0 }} className={className} aria-label="Configurador Configurador">
