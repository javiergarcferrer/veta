...
import {
  configuratorEmbedSnippet, configuratorShareUrl, dealerInboxUrl, configuratorQrDataUrl,
} from '../../lib/configuratorEmbed.js';
 ...
 *                      handoff rides (lib/configuratorEmbed → configuratorQrDataUrl).
 *
...
export default function DealerKit({ slug, inboxToken, name, brandName = null }) {
  const shareUrl = slug ? configuratorShareUrl(slug) : '';
  const inboxUrl = inboxToken ? dealerInboxUrl(inboxToken) : '';
  const snippet = slug ? configuratorEmbedSnippet(slug, { brandName }) : '';
  ...
  useEffect(() => {
    ...
    configuratorQrDataUrl(shareUrl).then((url) => { if (alive) setQr(url); });
    return () => { alive = false; };
  }, [shareUrl]);
