 ...
 * (`lib/configurator/autoLink.js`) needs from a model's real 3D file: its height in
 ...
 * pure `armSideFromHeightMap` in `lib/configurator/autoLink.js`, pinned against real
 * Noka/Kashima maps by `tests/configuratorAutoLink.test.js`.
 */
import { loadConfiguratorModels } from './modelLoader.js';
import { autoUnitScale } from '../../lib/configurator/model.js';
import { armSideFromHeightMap } from '../../lib/configurator/autoLink.js';
...
export async function probeMeshBinding(card) {
  if (!card?.mesh) return {};
  const { modelFor } = await loadConfiguratorModels({ pieces: [{ mesh: card.mesh }] });
