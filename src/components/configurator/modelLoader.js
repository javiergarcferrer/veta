 ...
 * ONE implementation behind BOTH the configurator stage (ConfiguratorStage) and the WebAR
 * export (ConfiguratorArViewer), so AR places the SAME real meshes the preview shows —
...
import { stripLabelMeshes, stripShadowMeshes } from '../../lib/configurator/meshClean.js';
import { glbForPiece } from '../../assets/ligne-roset/models3d.js';
 ...
 *
 * v4 = v3 with the split now TOPOLOGICAL (lib/configurator/solidSplit): a part must be a
 ...
 * `modelFor(piece)` is the selector `buildConfiguratorGroup` expects.
 */
/**
 ...
 * draw, and `buildConfiguratorGroup` clones it before touching anything, so the same
...
export async function loadConfiguratorModels(scene3d, cache = sharedModelCache) {
...
export function prewarmConfiguratorEngine() {
