 ...
 *
 * It turns a `resolveConfiguratorScene` spec into a furniture THREE.Group: each piece is
 * a pile of RoundedBoxes (lib/configurator/configuratorModel) sized to its real footprint,
 ...
 */
import { configuratorParts, autoUnitScale, meshSeamBleed } from '../../lib/configurator/model.js';
import {
...
import { traceGridLoops } from '../../lib/configurator/meshToPlan.js';
...
export const STANDARD_CONFIGURATOR_FINISH = {
 ...
 * configuratorModel.configuratorParts; these are the fine surface character ON those cushions).
 ...
 */
export function buildConfiguratorGroup(deps, scene3d, opts = {}) {
  ...
  for (const piece of (scene3d?.pieces || [])) {
    ...
    if (real && real.object) {
      ...
      for (const part of configuratorParts(piece.widthCm, piece.depthCm, piece.form)) {
        let mesh;
        if (part.shape === 'ridge') {
 ...
 * PURE half of the GPU-silhouette pipeline (ConfiguratorStage renders the GL half).
 *
...
export function setupConfiguratorStage(deps, renderer, scene, radius, { grid = false, ground = STAGE_GROUND, presentation = false } = {}) {
  ...
  // taps — see contactShadow for why no dial on it could ever soften). 2D
