...
import { partKeysFor, bodySignature, accessoryRoleFor } from '../../lib/togo/meshParts.js';
...
export async function harvestModelGroups(mesh, upAxis) {
  const { modelFor } = await loadTogoModels({ pieces: [{ mesh }] });
...
export async function buildAccessoryRefs(accessories) {
  ...
  const refs = await Promise.all((accessories || []).map(async (a) => {
    try {
      const { modelFor } = await loadTogoModels({ pieces: [{ mesh: a.mesh }] });
