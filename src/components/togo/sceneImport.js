 ...
 * floor plane (lib/togo/sceneSplit — pure, tested), and re-export each piece as
 * its own compact GLB ready for `uploadTogoMesh` + a togo_models row. three and
 ...
 * `generateBoxUvs` (togoSceneBuilder), which box-projects the fabric UVs from
...
function splitWorkerInstance() {
  ...
  splitWorker = new Worker(new URL('../../lib/togo/solidSplit.worker.js', import.meta.url), { type: 'module' });
 ...
 * (lib/togo/meshParts.js `partKeyFor` — pCon gives a part type one material id
 ...
 * bandwidth and nothing else. Deliberately NOT routed through `loadTogoModels`:
