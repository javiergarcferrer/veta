...
import { buildTogoGroup, setupTogoStage, disposeGroup, makeFabricMaps, sampleSwatchColor, fabricAnisotropy, STANDARD_TOGO_FINISH } from './togoSceneBuilder.js';
import { loadFabricAppearance, makeAppearanceCache } from './togoFabricAppearance.js';
...
export function togoThumbStoreKey(model, { code = '', fab = null, width, height } = {}) {
  ...
  return `/__togo-thumb/${encodeURIComponent(model.id)}/${w}x${h}/${encodeURIComponent(code || '-')}/${stamp}.png`;
}
 ...
 *  (see renderTogoThumb): a warm catalogue then paints without queueing behind
...
async function getEngine() {
  ...
  enginePromise = (async () => {
    ...
    // PCF, not PCFSoft — same reason as TogoStage: three r184 deprecated
 ...
 * On the opening screen every tile in range fires its own `renderTogoThumb`
...
export function renderTogoThumb(model, opts = {}) {
  return resolveTogoThumb(model, opts);
}
...
async function resolveTogoThumb(model, opts) {
  ...
  const job = (async () => {
    const stored = await readStoredThumb(togoThumbStoreKey(model, opts));
    if (stored) {
    ...
    return enqueueRender(() => _renderTogoThumb(model, opts));
  })();
...
async function _renderTogoThumb(model, opts = {}) {
  if (!model?.id) return null;
  ...
  // Re-checked HERE and not only in resolveTogoThumb: several tiles can queue the
  ...
  // the `finally` — same contract `_renderTogoSceneThumb` already keeps.
  renderer.setSize(frameW, frameH);
  camera.aspect = frameW / frameH;
  camera.updateProjectionMatrix();
  try {
    const loaded = await loadTogoModels({ pieces: [piece] });
    ...
    if (img?.url) {
      ...
      writeStoredThumb(togoThumbStoreKey(model, opts), img.blob);
    }
    return img?.url ?? null;
  } catch {
    return null;
 ...
 */
export function useTogoThumbnails(models, frame = null) {
 ...
 */
export function useTogoFabricThumbs(rows, frame = null) {
  const [thumbs, setThumbs] = useState({});
  const size = frameKey(frame);
  ...
  useEffect(() => {
    ...
    (async () => {
      for (const r of (rows || [])) {
        ...
        const url = await renderTogoThumb(r.model, { ...(frame || {}), code: r.code, fab: r.fab || null });
        if (!alive) return;
...
// never running alongside a `renderTogoThumb` call.
export function renderTogoSceneThumb(scene3d, opts = {}) {
  const pieces = scene3d?.pieces || [];
  if (!pieces.length) return Promise.resolve(null);
  return enqueueRender(() => _renderTogoSceneThumb(pieces, opts));
}
...
async function _renderTogoSceneThumb(pieces, opts = {}) {
  ...
  let group = null;
  try {
    const loaded = await loadTogoModels({ pieces });
    group = buildTogoGroup(deps, { pieces }, {
      ...STANDARD_TOGO_FINISH, normalMap: quilt, grainMap: grain,
...
/** Hook: renders the FULL assembled layout (a `resolveTogoScene` spec — every
...
export function useTogoSceneSnapshot(scene3d) {
  ...
  useEffect(() => {
    ...
    (async () => {
      const u = await renderTogoSceneThumb(scene3d);
      if (alive) setUrl(u);
    })();
    return () => { alive = false; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
