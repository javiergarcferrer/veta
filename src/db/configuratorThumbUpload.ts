...
// configuratorMeshUpload.ts: same bucket, same public-URL shape, same best-effort
// cleanup on replace. The bytes come from the SAME renderer the widget would
// have used (`renderConfiguratorThumb`), so a baked tile and a live one are the same
 ...
 */
export async function uploadConfiguratorThumb(modelId: string, stamp: string, blob: Blob): Promise<string> {
...
export async function removeConfiguratorThumb(url: string | null | undefined): Promise<void> {
  const marker = `/${BUCKET}/`;
