 ...
 * twice, so the outward offset pushes the two visits in opposite directions and
 * the stroke draws an X across the empty gap; the band smoother is handed a
 ...
 * moves — which is exactly the "tangled scribble" the gold selection outline drew
 * over a multi-island selection.
 *
 ...
 */
export function traceGridLoops(occ, gw, gh, cx = 1, cz = 1, eps = Math.max(cx, cz) * 0.9) {
  ...
  const gxOf = (k) => Math.floor(k / stride);
  const gzOf = (k) => k % stride;
  const toPt = (k) => ({ x: gxOf(k) * cx, y: gzOf(k) * cz });
  ...
  for (const from of [...out.keys()]) {
    for (;;) {
      ...
      const loop = [from];
      let prev = from, cur = arr.pop(), guard = 0;
