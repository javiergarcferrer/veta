...
export function togoEmbedUrl(dealerSlug) {
  const origin = typeof location !== 'undefined' ? location.origin : '';
  const q = dealerHashParam(dealerSlug);
  return `${origin}/#/embed/togo${q ? `?${q}` : ''}`;
}
 ...
 */
export function togoShareUrl(dealerSlug) {
  const origin = typeof location !== 'undefined' ? location.origin : '';
...
export function togoEmbedModalUrl(dealerSlug) {
  const dealerQ = dealerSlug ? `&dealer=${encodeURIComponent(dealerSlug)}` : '';
  return `${togoEmbedUrl()}?ctx=modal${dealerQ}`;
}
 ...
 * (components/togo/TogoArViewer): code-split through `safeDynamicImport`, so no
 ...
 */
export async function togoQrDataUrl(url, { width = 320 } = {}) {
  if (!url) return '';
  try {
 ...
 */
export async function fetchTogoCatalog(dealerSlug) {
 ...
 * ride (togoThumbnails' Cache Storage store): paint from the device's last
 ...
 */
export async function fetchTogoCatalogCached(dealerSlug, { onRefresh } = {}) {
  const url = withQuery(endpoint(), { dealer: dealerSlug });
  const snapshot = await readCatalogSnapshot(url);
  if (!snapshot) {
    const fresh = await fetchTogoCatalog(dealerSlug);
 ...
 */
export async function fetchTogoPlanSvgs(ids) {
 ...
 * { ok } or throws.
 */
export async function submitTogoRequest(payload, dealerSlug) {
 ...
 * (mirrors fetchTogoCatalog's error idiom).
 */
 ...
 * (same idiom as submitTogoRequest).
 */
 ...
 */
export function reportTogoView({ estimateUsd = 0, pieces = 0, dealerSlug = '' } = {}) {
  // This ping exists ONLY to tell Meta — a team browser sends nothing at all.
  if (togoViewReported || isTeamBrowser()) return;
  togoViewReported = true;
  try {
    const body = {
