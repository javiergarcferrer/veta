...
import {
  resolveConfiguratorModels, resolveConfiguratorDxf, placementsFromPlaced, resolveRequestsBoard,
  resolveConfiguratorScene, scenePlacementsFromPlaced,
} from '../core/quote/index.js';
import EmptyState from '../components/EmptyState.jsx';
import ImageView from '../components/ImageView.jsx';
import CompositionView from '../components/quote/CompositionView.jsx';
import { useMeshPlans, applyMeshPlans } from '../components/configurator/useMeshPlans.js';
...
export default function ConfiguratorRequests() {
  const navigate = useNavigate();
  const { profileId } = useApp();

  const requests = useLiveQuery(
    () => (profileId
      ? db.configuratorRequests.where('profileId').equals(profileId).toArray()
      : Promise.resolve([])),
    [profileId], [],
  );
  ...
  // PROJECTED (PRODUCT_LIST_COLUMNS): resolveConfiguratorModels only prices off the
  ...
  const base = useMemo(() => resolveConfiguratorModels(models, products), [models, products]);
  ...
  const setStatus = useCallback(async (req, status) => {
    ...
    await db.configuratorRequests.update(req.id, { status, updatedAt: Date.now() });
  }, []);
...
function RequestCard({ row, planModels, resolvedById, svgById, busy, error, onQuote, onStatus }) {
  ...
  const downloadDxf = useCallback(() => {
    ...
    const { dxf, filename } = resolveConfiguratorDxf(placements, { name: row.name || 'solicitud' });
  ...
  const scene3d = useMemo(
    () => resolveConfiguratorScene(scenePlacementsFromPlaced(row.placed, planModels)),
  ...
  const downloadGlb = useCallback(async () => {
    ...
    try {
      ...
      a.download = `configurador-3d-${(row.name || 'solicitud').trim().replace(/\s+/g, '-')}.glb`;
      a.click();
