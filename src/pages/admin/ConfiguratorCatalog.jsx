...
} from '../../components/configurator/sceneImport.js';
...
import {
  resolveModelManager, resolveConfiguratorModelCards, resolveConfiguratorModels, configuratorPickerFamilies,
} from '../../core/quote/index.js';
import { useConfiguratorThumbnails } from '../../components/configurator/thumbnails.js';
import EmptyState from '../../components/EmptyState.jsx';
import Modal from '../../components/Modal.jsx';
import ModelStudio, { ProductBindModal, ACCEPT_3D as ACCEPT_3D_SET } from '../../components/configurator/ModelStudio.jsx';
...
// Exported so ConfiguratorWorkspace's workspace-wide drop target accepts the same set.
...
/**
 * The "Modelos" section of the Togo workspace (ConfiguratorWorkspace) — the
 ...
 */
export default function ConfiguratorModels({ droppedFile = null, onDroppedFileConsumed }) {
  ...
  const models = useLiveQuery(
    () => (profileId ? db.configuratorModels.where('profileId').equals(profileId).toArray() : Promise.resolve([])),
    [profileId], [],
  );
  ...
  const families = useMemo(() => configuratorPickerFamilies(products), [products]);
  ...
  const cards = useMemo(() => resolveConfiguratorModelCards(models, families), [models, families]);
  ...
  // propia); `resolveConfiguratorModels` carries what the CLIENT gets — notably `parts`
  ...
  const resolvedById = useMemo(
    () => resolveConfiguratorModels(previewable, products).resolvedById,
  ...
  // drag a row, or Alt+↑↓). It rides the same `planConfiguratorReorder` and the same
  ...
  const importSeeds = useCallback(async () => {
    ...
    const configuratorFams = groupFamilies(prods).filter((f) => /togo/i.test(f.name || ''));
    const autoRoot = (seed) => {
      const keys = (seed.match || []).filter((k) => k !== 'togo');
      const hit = configuratorFams.find((f) => { const n = (f.name || '').toLowerCase(); return keys.some((k) => k && n.includes(k)); });
    ...
    for (const s of seedRows) {
      ...
      await db.configuratorModels.put({
...
function AddModelModal({
  ...
  const pendingThumbs = useConfiguratorThumbnails(pendingThumbInput);
  ...
  const onFile = useCallback(async (file) => {
    ...
    setMeshUrl((prev) => { if (prev) removeConfiguratorMesh(prev); return null; });   // drop an orphan from a re-pick
    if (!isModel(file)) {
    ...
    try {
      ...
      if (split.pieces.length === 1) {
        ...
        const url = await uploadConfiguratorMesh(glbFile);
        const p = await loadMeshPlan(url, { upAxis: split.upAxis });
        if (!p?.svg) { setError('El modelo no produjo una planta legible. ¿Está en cm y de pie?'); removeConfiguratorMesh(url); setStage('idle'); return; }
        const srcUrl = isGlb ? null : await uploadConfiguratorMesh(file).catch(() => null);
  ...
  const onManyFiles = useCallback(async (files) => {
    ...
    setError(null); setNotice(null); setResult(null); setPlan(null); setScene(null);
    setMeshUrl((prev) => { if (prev) removeConfiguratorMesh(prev); return null; });
    setSource((prev) => { if (prev?.url) removeConfiguratorMesh(prev.url); return null; });
    setStage('parsing');
  ...
  // intake in ConfiguratorWorkspace) lands here and runs the same unified flow as a
  ...
  const save = async () => {
    ...
    try {
      await db.configuratorModels.put({
        ...
        // `resolveConfiguratorModels` and the togo-embed payload already gate on.
  ...
  const runImport = async () => {
    ...
    for (const piece of included) {
      ...
      try {
        ...
        const url = await uploadConfiguratorMesh(new File([blob], `${slug}.glb`, { type: 'model/gltf-binary' }));
        const piecePlan = await loadMeshPlan(url, { upAxis: axis });
        if (!piecePlan?.svg) { failed.push(piece.name); removeConfiguratorMesh(url); continue; }
        let sourceUrl = null;
        if (piece.sourceFile && !/\.(glb|gltf)$/i.test(piece.sourceName || '')) {
          if (!sourceUrls.has(piece.sourceFile)) {
            sourceUrls.set(piece.sourceFile, await uploadConfiguratorMesh(piece.sourceFile).catch(() => null));
          }
