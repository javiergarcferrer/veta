/**
 * Tests for src/lib/configurator/buildShare.js + the design URL it feeds
 * (src/lib/configuratorEmbed.js `configuratorHandoffUrl`) — the ONE link that carries a whole
 ...
 * `?lang` (resolveConfiguratorLocale), so a link without it reverts the recipient to
...
import { encodeBuild, decodeBuild, decodeRoomFromBuild } from '../src/lib/configurator/buildShare.js';
import { configuratorHandoffUrl } from '../src/lib/configuratorEmbed.js';
...
test('the design URL carries dealer, lang and the build — and nothing without a build', () => {
  ...
  const url = configuratorHandoffUrl('casa-roset', build, { lang: 'fr' });
  ...
  assert.equal(configuratorHandoffUrl('casa-roset', '', { lang: 'fr' }), '');
  // Optional everywhere: Alcover's own embed passes no dealer, and no lang is
  // still a valid link (the dealer default applies).
  assert.equal(configuratorHandoffUrl(null, build), `/configurator?build=${build}`);
});
...
test('a design link round-trips end to end: URL → the same plan', () => {
  ...
  const url = configuratorHandoffUrl('casa-roset', encodeBuild(PLACED, room), { lang: 'es' });
