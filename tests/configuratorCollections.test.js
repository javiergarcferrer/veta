...
} from '../src/lib/configurator/collections.js';
import { resolveModelManager } from '../src/core/quote/views/modelManager.js';
import { resolveConfiguratorModels, resolveConfiguratorModelCards, resolveCollectionMenu } from '../src/core/quote/views/configuratorView.js';
...
test('the PALETTE lists one Exclusif chip, not two', () => {
  const vm = resolveConfiguratorModels(SPLIT, []);
...
test('the model CARDS file both spellings under one collection', () => {
  const cards = resolveConfiguratorModelCards(SPLIT, []);
