...
import {
  ...
  configuratorThumbStoreKey, configuratorThumbStamp, bakedThumbUrl, TILE_THUMB, ROW_THUMB,
} from '../src/components/configurator/thumbnails.js';
...
import {
  ...
  buildConfiguratorComponents, buildConfiguratorModularSeed, resolveConfigurator, resolveConfiguratorModels,
  resolveConfiguratorModelCards, planConfiguratorReorder, configuratorPickerFamilies,
  placementsFromPlaced, placementsFromComponents, resolveConfiguratorDxf, lineHasConfiguratorPlan,
...
// (ConfiguratorEmbed `pickableMaterials`) — imported here so the OFFER and the PRICE
...
import { structureGroupsOf } from '../src/lib/configurator/meshParts.js';
...
import { focusMeshes, DRESSABLE_ROLE } from '../src/components/configurator/ConfiguratorStage.jsx';
import { modulesOf, isModularLine } from '../src/lib/modules.js';
import { isPricedComponent } from '../src/lib/constants.js';
import { CONFIGURATOR_PIECES } from '../src/assets/ligne-roset/pieces.js';
...
test('the generated Togo manifest carries the measured cm footprints', () => {
  assert.equal(CONFIGURATOR_PIECES.length, 5);
...
const resolved = {
  a: { id: 'a', label: 'Sillón', name: 'Configurador Armchair', reference: '15420000A', subtype: 'A', widthCm: 102, depthCm: 102, unitPrice: 1200, dimensions: '102×102 cm', baseFamily: { root: '15420000', family: 'TOGO' } },
  gb: { id: 'gb', label: 'Sofá', name: 'Configurador Settee', reference: '15430000A', subtype: 'A', widthCm: 174, depthCm: 102, unitPrice: 2600, dimensions: '174×102 cm', baseFamily: { root: '15430000', family: 'TOGO' } },
};
const placed = [
...
test('placed pieces build a MODULAR line whose subtotal === compoundSubtotal', () => {
  const seed = buildConfiguratorModularSeed(placed, resolved, ids());
...
test('a per-placement material overrides price/subtype/swatch and flows into the total', () => {
  ...
  const comps = buildConfiguratorComponents(withMat, r, ids());
...
test('module groups are unique per piece, and rotation rides on the plan', () => {
  const comps = buildConfiguratorComponents(placed, resolved, ids());
...
test('the plan component carries its collection so a promoted quote keeps its seam bleed', () => {
  ...
  const comps = buildConfiguratorComponents(
...
test('resolveConfiguratorModels prices each model at its cheapest grade, drops inactive/empty', () => {
  const products = [
  ...
  const { activeModels, resolvedById, svgById } = resolveConfiguratorModels(models, products);
...
test('resolveConfiguratorModelCards reads bound state from the row, enriches only when the catalog is loaded', () => {
  const models = [
  ...
  const cold = resolveConfiguratorModelCards(models, []);
  ...
  const warm = resolveConfiguratorModelCards(models, configuratorPickerFamilies(products));
...
test('collections: legacy rows read as Togo, palette lists them in order, the seed is named after the build', () => {
  ...
  const { resolvedById, collections } = resolveConfiguratorModels(models, []);
  ...
  const prado = buildConfiguratorModularSeed(
  ...
  const mixed = buildConfiguratorModularSeed(
 ...
 * EN ESTE MODELO. HAZLO AUTOMATICO». So `resolveConfiguratorModels` INJECTS it at read
 ...
 */
test('resolveConfiguratorModels injects the colección’s estructura into the models that lack one', () => {
  const products = [
  ...
  const { resolvedById } = resolveConfiguratorModels(models, products);
  ...
  assert.deepEqual(models, before, 'resolveConfiguratorModels mutates nothing');
  ...
  const bare = resolveConfiguratorModels(noPalette, products).resolvedById;
...
test('resolveConfiguratorModelCards: collection + the configurator-shaped mesh ride on the card', () => {
  const models = [
  ...
  const cards = resolveConfiguratorModelCards(models, []);
  assert.equal(cards[0].collection, 'Prado');
  assert.equal(cards[1].collection, 'Togo', 'legacy rows file under Togo');
  // The card's mesh is the SAME shape the palette/renderConfiguratorThumb consume, so
...
test('configuratorPickerFamilies is empty until the catalog loads, then lists Togo families first', () => {
  assert.deepEqual(configuratorPickerFamilies(null), []);
  assert.deepEqual(configuratorPickerFamilies(undefined), []);
  const products = [
  ...
  const fams = configuratorPickerFamilies(products);
...
test('resolveConfigurator mirrors compoundSubtotal and lays tiles out in px', () => {
  ...
  assert.equal(vm.subtotalUsd, compoundSubtotal({ components: buildConfiguratorComponents(placed, resolved, ids()) }));
...
test('a free-rotated piece keeps its AABB tile and its angle through the quote round-trip', () => {
  ...
  const comps = buildConfiguratorComponents(rotated, resolved, ids());
...
test('resolveConfiguratorDxf builds a named DXF from the configurator placements', () => {
  ...
  const { dxf, filename, count } = resolveConfiguratorDxf(placements, { name: 'María / López' });
...
test('placementsFromComponents replays a promoted quote line, lineHasConfiguratorPlan detects it', () => {
  const seed = buildConfiguratorModularSeed(placed, resolved, ids());
  const line = { components: seed.components };
  assert.equal(lineHasConfiguratorPlan(line), true);
  assert.equal(lineHasConfiguratorPlan({ components: [{ name: 'Sofá' }] }), false);
...
test('per-part SKUs: parts bill as their own components and every surface agrees', () => {
  ...
  let n = 0;
  const comps = buildConfiguratorComponents([byParts], resolvedById, () => `id${n++}`);
  ...
  const compsWhole = buildConfiguratorComponents([picked], resolvedById, ids());
  ...
  const rb2 = resolveConfiguratorModels(twoSingles, products).resolvedById;
  ...
  const r2 = resolveConfiguratorModels(unbound, products).resolvedById.m2;
  assert.equal(r2.partFamilies.cushion, null);
  const comps2 = buildConfiguratorComponents(
    [{ uid: 'u2', pieceId: 'm2', x: 0, y: 0, rot: 0 }], resolveConfiguratorModels(unbound, products).resolvedById, ids(),
  );
...
test('estructura: a tagged metal part is INVISIBLE to money — same price, same components', () => {
  ...
  for (const [label, over, expected] of [
    ...
    assert.deepEqual(
      strip(buildConfiguratorComponents([legs], resolvedById, ids())),
      strip(buildConfiguratorComponents([plain], resolvedById, ids())),
      `components (${label})`,
    );
  }
  ...
  assert.equal(placementTotalUsd(picked, resolvedById), 1000);
  const comps = buildConfiguratorComponents([picked], resolvedById, ids());
...
//
// `focusMeshes` is the pure core of ConfiguratorStage's `focusMeshesFor` — the ONE
...
test('seat-mounted accessories float: mounted at height, never colliding', () => {
  ...
  const { resolvedById } = resolveConfiguratorModels(models, []);
  const placed = [
  ...
  const comps = buildConfiguratorComponents(placed, resolvedById, ids());
...
test('placementBreakdown itemizes base + parts and always foots to placementTotalUsd', () => {
  ...
  const solo = placementBreakdown({ uid: 'u2', pieceId: 'm2', x: 0, y: 0, rot: 0 }, resolveConfiguratorModels(plain, products).resolvedById);
...
test('NO-VANISH: a part picked at a grade its SKU ladder never offered keeps its line', () => {
  const { resolvedById } = resolveConfiguratorModels(NOVANISH_MODELS, NOVANISH_PRODUCTS);
...
test('NO-VANISH: the quote seed keeps the componente too, at 0, naming the grade', () => {
  const { resolvedById } = resolveConfiguratorModels(NOVANISH_MODELS, NOVANISH_PRODUCTS);
  const comps = buildConfiguratorComponents([NOVANISH_PLACED], resolvedById, ids());
  ...
  const rb = resolveConfiguratorModels(unbound, NOVANISH_PRODUCTS).resolvedById;
  assert.equal(buildConfiguratorComponents([{ uid: 'u2', pieceId: 'nb', x: 0, y: 0, rot: 0 }], rb, ids()).length, 1);
});
...
/** The widget's material wall, verbatim: ConfiguratorEmbed's `pickableMaterials`
...
test('WHOLE PIECE: the wall only offers telas the targeted model’s own ladder can price', () => {
  const { resolvedById, families } = resolveConfiguratorModels(WHOLEPIECE_MODELS, WHOLEPIECE_PRODUCTS);
  ...
  const bare = resolveConfiguratorModels([{ ...WHOLEPIECE_MODELS[0], id: 'bare', productRoot: null }], WHOLEPIECE_PRODUCTS);
  ...
  const solo = resolveConfiguratorModels(
...
test('WHOLE PIECE: a stored off-ladder pick prices NOTHING — never the model’s cheapest grade', () => {
  const { resolvedById } = resolveConfiguratorModels(WHOLEPIECE_MODELS, WHOLEPIECE_PRODUCTS);
  ...
  // and the build can't be handed on as a finished quote.
  const comps = buildConfiguratorComponents([WHOLEPIECE_PLACED], resolvedById, ids());
...
test('WHOLE PIECE: an on-ladder grade prices exactly as before, and no pick at all is never gated', () => {
  ...
  assert.equal(buildConfiguratorComponents([ok], resolvedById, ids())[0].unitPrice, 11595);
  ...
  const noFam = resolveConfiguratorModels([{ ...WHOLEPIECE_MODELS[0], id: 'nf', productRoot: null }], WHOLEPIECE_PRODUCTS);
...
test('ottoman zones: bicolor re-grades the base SKU (dearest zone wins) and never bills a part line', () => {
  ...
  let n = 0;
  const comps = buildConfiguratorComponents([bi], resolvedById, () => `id${n++}`);
...
test('the launch hero is the catalogue headline piece, in Festa Bleu Paon', () => {
  ...
  // The scene is a one-piece resolveConfiguratorScene result the turntable renders
...
// ── palette order (planConfiguratorReorder) ─────────────────────────────────────────
...
const orderOf = (rows) => resolveConfiguratorModelCards(rows, []).map((c) => c.id).join('');
const applyWrites = (rows, writes) => rows.map((r) => {
  const w = writes.find((x) => x.id === r.id);
  return w ? { ...r, sortOrder: w.sortOrder } : r;
});

test('planConfiguratorReorder moves a piece within its collection, writing only what moved', () => {
  const cards = resolveConfiguratorModelCards(ORDER_ROWS, []);
  const writes = planConfiguratorReorder(cards, { id: 'a', toIndex: 1 });
...
test('planConfiguratorReorder drops a piece at an arbitrary index (drag), not just one slot', () => {
  const cards = resolveConfiguratorModelCards(ORDER_ROWS, []);
  assert.equal(orderOf(applyWrites(ORDER_ROWS, planConfiguratorReorder(cards, { id: 'a', toIndex: 2 }))), 'bcade');
  assert.equal(orderOf(applyWrites(ORDER_ROWS, planConfiguratorReorder(cards, { id: 'c', toIndex: 0 }))), 'cabde');
});

test('planConfiguratorReorder never moves a piece out of its collection', () => {
  const cards = resolveConfiguratorModelCards(ORDER_ROWS, []);
  // 'e' is last of Prado; dragging past the end clamps inside the group, and the
  // other collection keeps its own order.
  const after = applyWrites(ORDER_ROWS, planConfiguratorReorder(cards, { id: 'e', toIndex: 9 }));
  assert.equal(orderOf(after), 'abcde');
  const up = applyWrites(ORDER_ROWS, planConfiguratorReorder(cards, { id: 'e', toIndex: 0 }));
  assert.equal(orderOf(up), 'abced', 'Prado reorders; the legacy Togo group is untouched');
});

test('planConfiguratorReorder defragments legacy ties instead of wedging on them', () => {
  ...
  const cards = resolveConfiguratorModelCards(zeros, []);
  const first = cards.filter((c) => c.collection === 'Togo')[0];
  const writes = planConfiguratorReorder(cards, { id: first.id, toIndex: 1 });
  const after = resolveConfiguratorModelCards(applyWrites(zeros, writes), []);
...
test('planConfiguratorReorder is a no-op when nothing moves', () => {
  const cards = resolveConfiguratorModelCards(ORDER_ROWS, []);
  assert.deepEqual(planConfiguratorReorder(cards, { id: 'a', toIndex: 0 }), []);
  assert.deepEqual(planConfiguratorReorder(cards, { id: 'nope', toIndex: 2 }), []);
  assert.deepEqual(planConfiguratorReorder(cards, {}), []);
});
...
test('the browser gate accepts exactly the envelope the Edge Function decodes', () => {
  ...
  const SHAPES = [
    ...
    ['an https: URL', 'https://cdn.alcover.do/configuratorsnap-abc.png', false],
...
test('the submit path asks the renderer for a DATA URL and ships only what the gate returned', () => {
  ...
  // watched this ship.
  const src = readFileSync(new URL('../src/pages/embed/ConfiguratorEmbed.jsx', import.meta.url), 'utf8').replace(/\s+/g, ' ');
  assert.match(
    src, /renderConfiguratorSceneThumb\([^)]*encode: 'dataUrl'[^)]*\)/,
 ...
 * replaced, never edited in place (`uploadConfiguratorMesh` mints a fresh randomUUID
...
test('the thumbnail store key changes with EVERYTHING that changes the picture', () => {
  const base = configuratorThumbStoreKey(STORE_MODEL, TILE_THUMB);
  assert.ok(base, 'a model with an id must be storable');
  assert.equal(base, configuratorThumbStoreKey({ ...STORE_MODEL }, TILE_THUMB), 'the same row must key the same way, or nothing ever hits');
  ...
  for (const [label, patch] of EDITS) {
    assert.notEqual(
      configuratorThumbStoreKey({ ...STORE_MODEL, ...patch }, TILE_THUMB), base,
      `${label} must mint a new key — the stored PNG no longer shows this piece`,
    );
  }

  // The frame and the cloth are part of the picture too.
  assert.notEqual(configuratorThumbStoreKey(STORE_MODEL, ROW_THUMB), base, 'two frames of one piece are two images, not a hit at the wrong size');
  assert.notEqual(configuratorThumbStoreKey(STORE_MODEL, { ...TILE_THUMB, code: 'K1234' }), base, 'a fabricked render is not the bare body');
  assert.notEqual(
    configuratorThumbStoreKey(STORE_MODEL, { ...TILE_THUMB, code: 'K1234', fab: { textureUrl: 'https://cdn.example/a.jpg' } }),
    configuratorThumbStoreKey(STORE_MODEL, { ...TILE_THUMB, code: 'K1234', fab: { textureUrl: 'https://cdn.example/b.jpg' } }),
    're-photographed cloth must repaint — the weave is what the render shows',
  );
  assert.equal(configuratorThumbStoreKey({ ...STORE_MODEL, id: null }, TILE_THUMB), null, 'no id, nothing to key on, nothing stored');
});
...
test('the bake stamp is the SAME on both sides — row shape and payload shape', () => {
  assert.equal(configuratorThumbStamp(BAKE_ROW), configuratorThumbStamp(BAKE_PAYLOAD));
  assert.equal(configuratorThumbStoreKey(BAKE_ROW, TILE_THUMB), configuratorThumbStoreKey(BAKE_PAYLOAD, TILE_THUMB));
  ...
  assert.equal(
    configuratorThumbStamp({ ...BAKE_ROW, collection: null, meshRotateY: null, meshUpAxis: null }),
    configuratorThumbStamp(BAKE_PAYLOAD),
  );
  // A piece with no mesh at all (procedural) still stamps, and stamps the same.
  assert.equal(
    configuratorThumbStamp({ ...BAKE_ROW, meshUrl: null }),
    configuratorThumbStamp({ ...BAKE_PAYLOAD, mesh: null }),
  );
});

test('a baked picture is used only for the piece, size and cloth it was baked for', () => {
  const stamp = configuratorThumbStoreKey(BAKE_PAYLOAD, TILE_THUMB);
  ...
  const hero = {
    ...
    heroThumbStamp: configuratorThumbStoreKey(BAKE_PAYLOAD, heroOpts),
  };
...
test('the store key is a bounded same-origin path that leaks no storage URL', () => {
  const key = configuratorThumbStoreKey(STORE_MODEL, TILE_THUMB);
  ...
  assert.match(key, /^\/__configurator-thumb\//, 'the key must be a same-origin path');
...
test('the catalogue tiles render at their measured box, and the lead snapshot does NOT', () => {
  ...
  // must never reach it.
  assert.match(
    src, /renderConfiguratorSceneThumb\(\s*scene3d,\s*\{\s*width:\s*960,\s*height:\s*600,\s*encode:\s*'dataUrl'\s*\}\s*\)/,
    'the lead snapshot must stay 960x600 — it is the picture the dealer quotes from',
  );
  assert.ok(
    !/renderConfiguratorSceneThumb\([^)]*TILE_THUMB/.test(src) && !/renderConfiguratorSceneThumb\([^)]*ROW_THUMB/.test(src),
...
test('MODO PIEZA prices the model\'s own SKU alone — the componentes are included', () => {
  const { resolvedById } = resolveConfiguratorModels(EXCLUSIF_MODEL, EXCLUSIF_PRODUCTS);
  const p = {
...
//   the quote        buildConfiguratorComponents      (what the dealer receives)
//
...
function surfaces(p, resolvedById) {
  ...
  const comps = buildConfiguratorComponents([p], resolvedById, ids());
  return {
...
test('COHERENCIA · vestir la pieza entera: money, «vestida», resumen y semilla se mueven juntos', () => {
  const { resolvedById } = resolveConfiguratorModels(COH_MODELS, COH_PRODUCTS);
...
test('COHERENCIA · el plan de una cotización replay-ea con el MISMO modo', () => {
  ...
  const r = resolvedById.sofa;
  const parts = planModeSwitch({ uid: 'u1', pieceId: 'sofa', x: 0, y: 0, rot: 0, material: CLOTH('A', 'X1') }, r, 'parts');
  const comps = buildConfiguratorComponents([parts], resolvedById, ids());
