...
test('dealerCollectionScope folds case + trims, and drops what is not a name', () => {
  ...
  assert.deepEqual([...dealerCollectionScope({ collections: [{}, 42, null, 'Configurador'] })], ['configurador']);
});
...
test('PARITY of the fold with the quote seed: the same fixture, the same two numbers', () => {
  // The fixture tests/configuratorQuote.test.js pins the client↔worker parity with — the
