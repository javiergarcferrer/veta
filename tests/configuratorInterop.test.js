/**
 * Tests for lib/configurator/interopMesh — the INTEROP contract of every 3D file that
 ...
 *      required extension — pinned here as a source scan of glbExport.
 */
...
import { dequantizedFloats, glbRequiredExtensions } from '../src/lib/configurator/interopMesh.js';
...
test('glbExport — exportGlbBlob dequantizes and refuses non-interop output', () => {
  const src = readFileSync(join(ROOT, 'src/components/configurator/glbExport.js'), 'utf8');
  ...
  assert.ok(/dequantizedFloats/.test(src),
    'the decode must come from lib/configurator/interopMesh (one home, tested above)');
...
test('ONE 3D export — the geometry-only OBJ path is gone from every surface', () => {
  ...
  for (const rel of ['src/pages/embed/ConfiguratorEmbed.jsx', 'src/pages/DealerInbox.jsx', 'src/pages/ConfiguratorRequests.jsx']) {
